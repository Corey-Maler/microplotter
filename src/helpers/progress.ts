export class Progress {
  ...
  private printCurrentPercentage() {
    ...
    process.stdout.write(
      `${this.label}: ${(this.percentage * 100).toFixed(2)}% Processed ${this.currentIteration} out of ${this.iterations}. Avg time per iteration: ${(avg).toFixed(2)}ms (${((1 / avg) * 1000).toFixed(2)} per second)`,
    );
  }
